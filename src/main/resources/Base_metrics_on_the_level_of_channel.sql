...
from (
  select  row_number() over (partition by subscriptionid order  by dt_local_tz desc ) row_num  ,a.* from [my_dataset.stg2_${PRODUCTNAME}_user_activity_history] a
  where date(dt_local_tz) <= '${current_start_date_bases}'
  ) a
left join  public.baas_subscription_status b on a.currentstatus=b.subscription_status_id
where a.row_num=1 group by  1,5,6,7,8